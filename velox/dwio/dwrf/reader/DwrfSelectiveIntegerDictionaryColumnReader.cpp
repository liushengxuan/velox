...
DwrfSelectiveIntegerDictionaryColumnReader::DwrfSelectiveIntegerDictionaryColumnReader(
...
//  TODO(Shengxuan Liu): Change the name back to convertRleVersion1() after removing unnecessary parts
  rleVersion_ = convertRleVersion1(encoding.kind());
...
uint64_t DwrfSelectiveIntegerDictionaryColumnReader::skip(uint64_t numValues) {
...
void DwrfSelectiveIntegerDictionaryColumnReader::read(
    vector_size_t offset,
    RowSet rows,
    const uint64_t* incomingNulls) {
  VELOX_WIDTH_DISPATCH(
      dwio::common::reader::sizeOfIntKind(type_->kind()), prepareRead, offset, rows, incomingNulls);
...
void DwrfSelectiveIntegerDictionaryColumnReader::ensureInitialized() {
