...
void SelectiveTimestampColumnReader::readHelper(RowSet rows) {
  ...
  // 'values_'
  dwio::common::reader::detail::ensureCapacity<uint64_t>(secondsValues_, numValues_, &memoryPool_);
...
void SelectiveTimestampColumnReader::getValues(RowSet rows, VectorPtr* result) {
  ...
  auto rawNulls = nullsInReadRange_
      ? (returnReaderNulls_ ? nullsInReadRange_->as<uint64_t>()
                            : rawResultNulls_)
      : nullptr;
  dwio::common::reader::detail::fillTimestamps(rawTs, rawNulls, secondsData, nanosData, numValues_);
  values_ = tsValues;
