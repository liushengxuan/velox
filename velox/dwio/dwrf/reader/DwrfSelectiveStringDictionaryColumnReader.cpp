...
#include "velox/dwio/dwrf/reader/DwrfSelectiveStringDictionaryColumnReader.h"

namespace facebook::velox::dwrf {

using namespace dwio::common;

DwrfSelectiveStringDictionaryColumnReader::DwrfSelectiveStringDictionaryColumnReader(
...
uint64_t DwrfSelectiveStringDictionaryColumnReader::skip(uint64_t numValues) {
...
void DwrfSelectiveStringDictionaryColumnReader::loadDictionary(
    SeekableInputStream& data,
    IntDecoder</*isSigned*/ false>& lengthDecoder,
    dwio::common::reader::DictionaryValues& values) {
...
void DwrfSelectiveStringDictionaryColumnReader::loadStrideDictionary() {
...
void DwrfSelectiveStringDictionaryColumnReader::makeDictionaryBaseVector() {
...
void DwrfSelectiveStringDictionaryColumnReader::read(
...
void DwrfSelectiveStringDictionaryColumnReader::getValues(
    RowSet rows,
    VectorPtr* result) {
...
void DwrfSelectiveStringDictionaryColumnReader::ensureInitialized() {
  if (LIKELY(initialized_)) {
    return;
  }

  dwio::common::reader::Timer timer;
  ...
  if (inDictionaryReader_) {
    ...
    positionOffset_ = notNullDecoder_
        ? std::dynamic_pointer_cast<ByteRleDecoder>(notNullDecoder_)->loadIndices(*index_, indexStartOffset)
        : indexStartOffset;
