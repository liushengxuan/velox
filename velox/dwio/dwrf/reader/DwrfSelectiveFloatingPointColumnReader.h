...
class DwrfSelectiveFloatingPointColumnReader : public DwrfSelectiveColumnReader {
 public:
  using ValueType = TRequested;
  DwrfSelectiveFloatingPointColumnReader(
...
template <typename TData, typename TRequested>
DwrfSelectiveFloatingPointColumnReader<TData, TRequested>::
    DwrfSelectiveFloatingPointColumnReader(
...
template <typename TData, typename TRequested>
uint64_t DwrfSelectiveFloatingPointColumnReader<TData, TRequested>::skip(
    uint64_t numValues) {
...
template <typename TData, typename TRequested>
template <typename TVisitor>
void DwrfSelectiveFloatingPointColumnReader<TData, TRequested>::readWithVisitor(
    RowSet rows,
    TVisitor visitor) {
  vector_size_t numRows = rows.back() + 1;
  if (nullsInReadRange_) {
    decoder_.template readWithVisitor<true, TVisitor>(
        DwrfSelectiveColumnReader::nullsInReadRange_->as<uint64_t>(), visitor);
  } else {
...
template <typename TData, typename TRequested>
template <typename TFilter, bool isDense, typename ExtractValues>
void DwrfSelectiveFloatingPointColumnReader<TData, TRequested>::readHelper(
    common::Filter* filter,
    RowSet rows,
    ExtractValues extractValues) {
  readWithVisitor(
...
template <typename TData, typename TRequested>
template <bool isDense, typename ExtractValues>
void DwrfSelectiveFloatingPointColumnReader<TData, TRequested>::processFilter(
    common::Filter* filter,
    RowSet rows,
    ExtractValues extractValues) {
...
template <typename TData, typename TRequested>
template <bool isDense>
void DwrfSelectiveFloatingPointColumnReader<TData, TRequested>::processValueHook(
    RowSet rows,
    ValueHook* hook) {
  switch (hook->kind()) {
...
template <typename TData, typename TRequested>
void DwrfSelectiveFloatingPointColumnReader<TData, TRequested>::read(
