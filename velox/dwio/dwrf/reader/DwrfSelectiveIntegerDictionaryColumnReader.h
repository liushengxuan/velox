...
class DwrfSelectiveIntegerDictionaryColumnReader : public DwrfSelectiveColumnReader {
 public:
  using ValueType = int64_t;

  DwrfSelectiveIntegerDictionaryColumnReader(
...
template <typename ColumnVisitor>
void DwrfSelectiveIntegerDictionaryColumnReader::readWithVisitor(
    RowSet rows,
...
template <typename TFilter, bool isDense, typename ExtractValues>
void DwrfSelectiveIntegerDictionaryColumnReader::readHelper(
    common::Filter* filter,
    RowSet rows,
    ExtractValues extractValues) {
  switch (valueSize_) {
...
template <bool isDense, typename ExtractValues>
void DwrfSelectiveIntegerDictionaryColumnReader::processFilter(
    common::Filter* filter,
    ExtractValues extractValues,
    RowSet rows) {
...
void DwrfSelectiveIntegerDictionaryColumnReader::processValueHook(
    RowSet rows,
    ValueHook* hook) {
  switch (hook->kind()) {
