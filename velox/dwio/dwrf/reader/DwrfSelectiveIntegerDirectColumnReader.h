...
class DwrfSelectiveIntegerDirectColumnReader : public DwrfSelectiveColumnReader {
 public:
  using ValueType = int64_t;

  DwrfSelectiveIntegerDirectColumnReader(
...
template <typename ColumnVisitor>
void DwrfSelectiveIntegerDirectColumnReader::readWithVisitor(
    RowSet rows,
...
template <typename TFilter, bool isDense, typename ExtractValues>
void DwrfSelectiveIntegerDirectColumnReader::readHelper(
    common::Filter* filter,
    RowSet rows,
    ExtractValues extractValues) {
  switch (valueSize_) {
...
template <bool isDense, typename ExtractValues>
void DwrfSelectiveIntegerDirectColumnReader::processFilter(
    common::Filter* filter,
    ExtractValues extractValues,
    RowSet rows) {
...
void DwrfSelectiveIntegerDirectColumnReader::processValueHook(
    RowSet rows,
    ValueHook* hook) {
  switch (hook->kind()) {
