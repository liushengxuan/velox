...
#include "velox/dwio/dwrf/reader/DwrfSelectiveColumnReaderInternal.h"
...
class DwrfSelectiveRepeatedColumnReader : public DwrfSelectiveColumnReader {
 public:
 ...
 protected:
  ...
  DwrfSelectiveRepeatedColumnReader(
...
//    TODO(Shengxuan Liu): Change the funtion back to convertRleVersion()
    auto rleVersion = convertRleVersion1(stripe.getEncoding(encodingKey).kind());
  ...
  void makeNestedRowSet(RowSet rows) {
    ...
    dwio::common::reader::detail::ensureCapacity<vector_size_t>(offsets_, rows.size(), &memoryPool_);
    dwio::common::reader::detail::ensureCapacity<vector_size_t>(sizes_, rows.size(), &memoryPool_);
...
class DwrfSelectiveListColumnReader : public DwrfSelectiveRepeatedColumnReader {
 public:
  DwrfSelectiveListColumnReader(
  ...
  void seekToRowGroup(uint32_t index) override {
    ...
    if (notNullDecoder_) {
      std::dynamic_pointer_cast<ByteRleDecoder>(notNullDecoder_)->seekToRowGroup(positionsProvider);
    }
 ...
 private:
  std::unique_ptr<DwrfSelectiveColumnReader> child_;
  const std::shared_ptr<const dwio::common::TypeWithId> requestedType_;
};

class SelectiveMapColumnReader : public DwrfSelectiveRepeatedColumnReader {
 public:
 ...
 private:
  std::unique_ptr<DwrfSelectiveColumnReader> keyReader_;
  std::unique_ptr<DwrfSelectiveColumnReader> elementReader_;
  const std::shared_ptr<const dwio::common::TypeWithId> requestedType_;
};
