...
class DwrfSelectiveByteRleColumnReader : public DwrfSelectiveColumnReader {
 public:
  using ValueType = int8_t;

  DwrfSelectiveByteRleColumnReader(
...
template <typename ColumnVisitor>
void DwrfSelectiveByteRleColumnReader::readWithVisitor(
    RowSet rows,
...
template <typename TFilter, bool isDense, typename ExtractValues>
void DwrfSelectiveByteRleColumnReader::readHelper(
    common::Filter* filter,
    RowSet rows,
    ExtractValues extractValues) {
  readWithVisitor(
...
template <bool isDense, typename ExtractValues>
void DwrfSelectiveByteRleColumnReader::processFilter(
    common::Filter* filter,
    ExtractValues extractValues,
    RowSet rows) {
...
void DwrfSelectiveByteRleColumnReader::processValueHook(
    RowSet rows,
    ValueHook* hook) {
  ...
  switch (hook->kind()) {
    case aggregate::AggregationHook::kSumBigintToBigint:
      readHelper<common::AlwaysTrue, isDense>(
          &dwio::common::reader::alwaysTrue(), rows, ExtractToHook<SumHook<int64_t, int64_t>>(hook));
      break;
    default:
      readHelper<common::AlwaysTrue, isDense>(
          &dwio::common::reader::alwaysTrue(), rows, ExtractToGenericHook(hook));
  }
}
