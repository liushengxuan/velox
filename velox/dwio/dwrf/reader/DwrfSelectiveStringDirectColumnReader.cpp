...
#include "velox/dwio/dwrf/reader/DwrfSelectiveStringDirectColumnReader.h"

namespace facebook::velox::dwrf {

DwrfSelectiveStringDirectColumnReader::DwrfSelectiveStringDirectColumnReader(
    const std::shared_ptr<const dwio::common::TypeWithId>& nodeType,
    StripeStreams& stripe,
    common::ScanSpec* scanSpec,
    FlatMapContext flatMapContext)
    : DwrfSelectiveColumnReader(
          nodeType,
          stripe,
          scanSpec,
          nodeType->type,
...
//  TODO(Shengxuan Liu): change convertRleVersion1() back to convertRleVersion()
  RleVersion rleVersion =
      convertRleVersion1(stripe.getEncoding(encodingKey).kind());
...
uint64_t DwrfSelectiveStringDirectColumnReader::skip(uint64_t numValues) {
  numValues = ColumnReader::skip(numValues);
  dwio::common::reader::detail::ensureCapacity<int64_t>(lengths_, numValues, &memoryPool_);
...
void DwrfSelectiveStringDirectColumnReader::extractCrossBuffers(
...
inline void DwrfSelectiveStringDirectColumnReader::makeSparseStarts(
    int32_t startRow,
    const int32_t* rows,
    int32_t numRows,
    int32_t* starts) {
...
void DwrfSelectiveStringDirectColumnReader::extractNSparse(
    const int32_t* rows,
    int32_t row,
    int32_t numValues) {
  int32_t starts[8];
...
inline bool DwrfSelectiveStringDirectColumnReader::try8Consecutive(
    int32_t start,
    const int32_t* rows,
    int32_t row) {
...
void DwrfSelectiveStringDirectColumnReader::extractSparse(
    const int32_t* rows,
    int32_t numRows) {
  rowLoop(
      rows,
...
void DwrfSelectiveStringDirectColumnReader::skipInDecode(
    int32_t numValues,
    int32_t current,
...
folly::StringPiece DwrfSelectiveStringDirectColumnReader::readValue(
    int32_t length) {
  skipBytes(bytesToSkip_, blobStream_.get(), bufferStart_, bufferEnd_);
  bytesToSkip_ = 0;
...
void DwrfSelectiveStringDirectColumnReader::decode(
    const uint64_t* nulls,
    Visitor visitor) {
...
void DwrfSelectiveStringDirectColumnReader::readWithVisitor(
    RowSet rows,
    TVisitor visitor) {
  ...
  constexpr bool isExtract =
      ...
      std::is_same<
          ...
          ExtractToReader<DwrfSelectiveStringDirectColumnReader>>::value;
...
void DwrfSelectiveStringDirectColumnReader::readHelper(
...
void DwrfSelectiveStringDirectColumnReader::processFilter(
    common::Filter* filter,
    RowSet rows,
    ExtractValues extractValues) {
...
void DwrfSelectiveStringDirectColumnReader::read(
  ...
  dwio::common::reader::detail::ensureCapacity<int32_t>(lengths_, end - numNulls, &memoryPool_);
    ...
    if (scanSpec_->valueHook()) {
      if (isDense) {
        readHelper<common::AlwaysTrue, true>(
            &dwio::common::reader::alwaysTrue(), rows, ExtractToGenericHook(scanSpec_->valueHook()));
      } else {
