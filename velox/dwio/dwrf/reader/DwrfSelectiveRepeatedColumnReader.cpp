...
#include "velox/dwio/dwrf/reader/DwrfSelectiveRepeatedColumnReader.h"

namespace facebook::velox::dwrf {

DwrfSelectiveListColumnReader::DwrfSelectiveListColumnReader(
  ...
  VELOX_CHECK(
      ...
      "DwrfSelectiveListColumnReader must select the values stream");
  ...
  child_ = DwrfSelectiveColumnReader::build(
      childType,
      nodeType_->childAt(0),
      stripe,
...
uint64_t DwrfSelectiveListColumnReader::skip(uint64_t numValues) {
...
void DwrfSelectiveListColumnReader::read(
...
void DwrfSelectiveListColumnReader::getValues(RowSet rows, VectorPtr* result) {
  compactOffsets(rows);
  VectorPtr elements;
  if (child_ && !nestedRows_.empty()) {
...
SelectiveMapColumnReader::SelectiveMapColumnReader(
  ...
  keyReader_ = DwrfSelectiveColumnReader::build(
      keyType,
      nodeType_->childAt(0),
      stripe,
      scanSpec_->children()[0].get(),
  ...
  elementReader_ = DwrfSelectiveColumnReader::build(
      valueType,
      nodeType_->childAt(1),
      stripe,
