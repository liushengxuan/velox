...
#include "velox/dwio/common/reader/SelectiveColumnReaderInternal.h"
...
using common::TypeWithId;
using common::typeutils::CompatChecker;

::facebook::velox::common::AlwaysTrue& alwaysTrue() {
...
SelectiveColumnReader::SelectiveColumnReader(
    ...
    memory::MemoryPool& memoryPool,
    const TypePtr& type)
    : ColumnReader(std::move(requestedType), memoryPool),
      scanSpec_(scanSpec),
      type_{type} {}
...
//        TODO: FilterResult is from ColumnVisitor. Move ColumVisitor to public
        dwrf::FilterResult::kUnknown,
        scanState_.filterCache.size());
  }
}

// To Dwrf: proto is for Dwrf only +++
